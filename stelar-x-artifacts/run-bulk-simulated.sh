...
Options:
  ...
  --simphy-data-dir DIR  Store/read datasets under DIR
...
SIMPHY_DATA_ARG=(--simphy-data-dir "$SIMPHY_DATA_DIR")
    ...
    for sb in "${SB_LIST[@]}"; do
      for spmin in "${SPMIN_LIST[@]}"; do
        for spmax in "${SPMAX_LIST[@]}"; do
          ...
          ./sim.sh -rs $NUM_REPLICATES $BASE_DIR_ARG "${SIMPHY_DATA_ARG[@]}" -t "$t" -g "$g" --sb "$sb" --spmin "$spmin" --spmax "$spmax" --fresh
          ...
          for ((i=1; i<=NUM_REPLICATES; i++)); do
            ...
            if [[ "$METHOD" == "stelar" ]]; then
              ./test-stelar-simulated.sh -r "R$i" $BASE_DIR_ARG "${SIMPHY_DATA_ARG[@]}" -t "$t" -g "$g" --sb "$sb" --spmin "$spmin" --spmax "$spmax" $FRESH_ARG
            else
              ...
              ./test-baseline-simulated.sh -r "R$i" $BASE_DIR_ARG "${SIMPHY_DATA_ARG[@]}" -t "$t" -g "$g" --sb "$sb" --spmin "$spmin" --spmax "$spmax" "${METHOD_ARGS[@]}" $FRESH_ARG
            fi
          done
