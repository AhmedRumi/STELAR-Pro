#!/usr/bin/env bash
# test-stelar-pro-simulated.sh
# Runs STELAR-Pro on a simulated dataset replicate and records the usual research
# statistics.
...
if [[ "$STELAR_PRO_ROOT_SET" == false ]]; then
  STELAR_PRO_ROOT="$SCRIPT_ROOT"
fi
...
RESULTS_DIR="${SIMPHY_RUN_DIR%/}/stelar-pro-outputs/${SETTING_NAME}"
STAT_FILE="${RESULTS_DIR%/}/stat-stelar-pro.csv"
LOCK_FILE="${RESULTS_DIR%/}/.stelar-pro.lock"
SUCCESS_FILE="${RESULTS_DIR%/}/.stelar-pro.success"
...
if [[ ! -f "$ALL_GT_FILE" ]]; then
  ...
  if [[ "$INCOMPLETE" == true ]]; then
    ...
    SIM_INC_CMD=("${STELAR_PRO_ROOT}/sim_incomplete.sh" -t "$TAXA_NUM" -g "$GENE_TREES" -rs "$REPLICATE_COUNT" --sb "$SB" --spmin "$SPMIN" --spmax "$SPMAX")
...
mkdir -p "${RESULTS_DIR%/}"
rm -f "$LOCK_FILE" "$SUCCESS_FILE" "$RUN_LOG" "$OUT_STELAR_PRO"
touch "$LOCK_FILE"
...
CMD=("${STELAR_PRO_ROOT}/run-stelar-pro-with-monitor.sh" -i "$ALL_GT_FILE" -o "$OUT_STELAR_PRO" --stelar-pro-root "$STELAR_PRO_ROOT" --no-notify)
...
if [[ -n "$STELAR_PRO_OPTS" ]]; then
  CMD+=(--opts "$STELAR_PRO_OPTS")
fi

echo "==> Running STELAR-Pro"
set +e
"${CMD[@]}" 2>&1 | tee "$RUN_LOG"
STELAR_PRO_EXIT_CODE=${PIPESTATUS[0]}
set -e
...
RF_RATE="NA"
if [[ -f "$OUT_STELAR_PRO" && -f "$TRUE_SPECIES_TREE" ]]; then
  rf_output=$("$PYTHON_BIN" "${STELAR_PRO_ROOT}/rf.py" "$OUT_STELAR_PRO" "$TRUE_SPECIES_TREE" 2>&1) || true
