        ...
        case "$alg" in
          stelar)
            ...
            if [[ ! -x "${STELAR_PRO_ROOT}/run.sh" ]]; then
              echo -e "${RED}Error: STELAR run.sh not found in STELAR_PRO_ROOT.${NC}"
              errors_found=true
            fi
            ;;
...
run_algorithm_and_write_stats() {
    ...
    if [[ -f "$OUT_FILE" ]]; then
      ...
      if [[ -f "${STELAR_PRO_ROOT%/}/rf.py" && -x "$(command -v python3)" ]]; then
        echo "      Calculating RF using ${STELAR_PRO_ROOT%/}/rf.py"
        rf_output=$(python3 "${STELAR_PRO_ROOT%/}/rf.py" "$OUT_FILE" "$TRUE_SPECIES_TREE" 2>&1) || rf_output="$rf_output"
...
if [[ -z "${RUN_WITH_MONITOR_SCRIPT}" ]]; then
  RUN_WITH_MONITOR_SCRIPT="${STELAR_PRO_ROOT%/}/run-with-monitor.sh"
fi
if [[ -z "${RUN_BASELINE_WITH_MONITOR_SCRIPT}" ]]; then
  RUN_BASELINE_WITH_MONITOR_SCRIPT="${STELAR_PRO_ROOT%/}/run-baseline-with-monitor.sh"
fi
